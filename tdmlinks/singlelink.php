...
$xoopsOption['template_main'] = 'tdmlinks_singlelink.tpl';
...
$navigation = $navigation . ' <img src="assets/images/deco/arrow.gif" alt="arrow" /> ' . $view_links->getVar('title');
...
if (is_object($xoopsUser) && $xoopsUser->isAdmin($xoopsModule->mid())) {
    $adminlink = '<a href="' . XOOPS_URL . '/modules/tdmlinks/admin/links.php?op=view_links&amp;links_lid=' . $_REQUEST['lid'] . '" title="' . _MD_TDMLINKS_EDITTHISDL . '"><img src="' . XOOPS_URL . '/modules/tdmlinks/assets/images/icon/edit.png" width="16px" height="16px" border="0" alt="' . _MD_TDMLINKS_EDITTHISDL . '" /></a>';
} else {
    $adminlink = '';
}
...
$xoopsTpl->assign('title', $view_links->getVar('title'));
...
if (($xoopsModuleConfig['usetellafriend'] == 1) and (is_dir('../tellafriend'))) {
    ...
    $tellafriend_texte = '<a target="_top" href="mailto:?subject=' . rawurlencode(sprintf(_MD_TDMLINKS_SINGLELINK_INTLINKFOUND, $xoopsConfig['sitename'])) . '&amp;body=' . rawurlencode(sprintf(_MD_TDMLINKS_SINGLELINK_INTLINKFOUND,
                $xoopsConfig['sitename']) . ':  ' . XOOPS_URL . '/modules/tdmlinks/singlelink.php?lid=' . $_REQUEST['lid']) . '">' . _MD_TDMLINKS_SINGLELINK_TELLAFRIEND . '</a>';
}
