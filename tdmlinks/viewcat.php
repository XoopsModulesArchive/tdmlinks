...
$xoopsOption['template_main'] = 'tdmlinks_viewcat.html';
...
//navigation
$nav_category = tdmlinks_PathTreeUrl($mytree, $cid, $linkscat_arr, 'cat_title', $prefix = ' <img src="images/deco/arrow.gif" alt="arrow" /> ', true, 'ASC');
...
foreach (array_keys($linkscat_arr) as $i) {
    if ($linkscat_arr[$i]->getVar('cat_pid') == $cid){
        ...
        $xoopsTpl->append('subcategories', array('image' => $uploadurl . $linkscat_arr[$i]->getVar('cat_imgurl'), 'id' => $linkscat_arr[$i]->getVar('cat_cid'), 'title' => $linkscat_arr[$i]->getVar('cat_title'), 'description_main' => $linkscat_arr[$i]->getVar('cat_description_main'), 'infercategories' => $subcategories, 'totallinks' => $totallinks, 'count' => $count));
        $count++;
    }
}
...
if ($xoopsModuleConfig['perpage'] > 0){
    ...
    foreach (array_keys($links_arr) as $i) {
        ...
        $xoopsTpl->append('file', array('id' => $links_arr[$i]->getVar('lid'),'cid' => $links_arr[$i]->getVar('cid'), 'title' => $links_arr[$i]->getVar('title'), 'new' => $new, 'pop' => $pop,'logourl' => $logourl,'updated' => $datetime,'description_short' => $description_short,
                                        'adminlink' => $adminlink, 'submitter' => $submitter, 'perm_link' => $perm_link, 'count' => $cpt));
        //pour les mots clef
    ...
    if($numrows>1){
        ...
        if ($sortorder == "hitsASC") $affichage_tri = _MD_TDMLINKS_CAT_POPULARITYLTOM;
        if ($sortorder == "hitsDESC") $affichage_tri = _MD_TDMLINKS_CAT_POPULARITYMTOL;
        if ($sortorder == "titleASC") $affichage_tri = _MD_TDMLINKS_CAT_TITLEATOZ;
        if ($sortorder == "titleDESC") $affichage_tri = _MD_TDMLINKS_CAT_TITLEZTOA;
        if ($sortorder == "dateASC") $affichage_tri = _MD_TDMLINKS_CAT_DATEOLD;
        if ($sortorder == "dateDESC") $affichage_tri = _MD_TDMLINKS_CAT_DATENEW;
        if ($sortorder == "ratingASC") $affichage_tri = _MD_TDMLINKS_CAT_RATINGLTOH;
        if ($sortorder == "ratingDESC") $affichage_tri = _MD_TDMLINKS_CAT_RATINGHTOL;
