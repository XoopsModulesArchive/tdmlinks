...
$xoopsOption['template_main'] = 'tdmlinks_ratelink.html';
...
switch ($op)
{
    ...
    case "liste":
        ...
        $navigation .= ' <img src="images/deco/arrow.gif" alt="arrow" /> ' . _MD_TDMLINKS_SINGLELINK_RATHLINK;
        ...
        if ($ratinguser != 0) {
            ...
            foreach (array_keys($links_arr) as $i) {
                if ($links_arr[$i]->getVar('submitter') == $ratinguser) {
                    redirect_header('singlelink.php?lid=' . intval($_REQUEST['lid']), 2, _MD_TDMLINKS_RATELINK_CANTVOTEOWN);
                    exit();
                }
            }
            ...
            foreach (array_keys($linksvotes_arr) as $i) {
                if ($linksvotes_arr[$i]->getVar('ratinguser') == $ratinguser) {
                    redirect_header('singlelink.php?lid=' . intval($_REQUEST['lid']), 2, _MD_TDMLINKS_RATELINK_VOTEONCE);
                    exit();
                }
            }
        } else {
