...
function Import_wflinks($shots='', $catimg='')
{
    ...
    if ( $ok == 1 ){
        ...
        while ($donnees = $xoopsDB->fetchArray($query_topic))
        {
            ...
            $insert = $xoopsDB->queryF("INSERT INTO ".$xoopsDB->prefix("tdmlinks_cat")." (cat_cid, cat_pid, cat_title, cat_imgurl, cat_description_main, cat_weight ) VALUES ('".$donnees['cid']."', '".$donnees['pid']."', '".$donnees['title']."', '".$img."', '".addcslashes($donnees['description'],"'")."', '".$donnees['weight']."')");
            if (!$insert) {
                echo "<font color='red'>" . _AM_TDMLINKS_IMPORT_ERROR_DATA .": </font> " . $donnees['title'] . "<br>";
            }
            echo sprintf(_AM_TDMLINKS_IMPORT_CAT_IMP . '<br/>', $donnees['title']);
